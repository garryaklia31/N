...
from gates.vantiv import vantiv
from time import perf_counter


@Client.on_message(filters.command("fu", PREFIXES))
async def fu(client: Client, m: Message):
    ...
    if not ccs:
        return await m.reply(
            "𝙂𝙖𝙩𝙚𝙬𝙖𝙮 <code>𝙁𝙪𝙘𝙠𝙚𝙧 ♻️ -» $5</code>\n𝙁𝙤𝙧𝙢𝙖𝙩 -» <code>/fu cc|month|year|cvc</code>",
            quote=True,
        )
    ini = perf_counter()
    cc = ccs[0]
    ...
    status, result = await vantiv(cc, mes, ano, cvv)
